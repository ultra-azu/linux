...
static int gcc_debug_set_enable(void *data, u64 val)
{
...
DEFINE_DEBUGFS_ATTRIBUTE(clk_enable_fops, NULL, gcc_debug_set_enable, "%llu\n");

static int gcc_debug_set_rate(void *data, u64 val)
{
	struct clk* clock = data;
...
DEFINE_DEBUGFS_ATTRIBUTE(clk_set_rate_fops, NULL, gcc_debug_set_rate, "%llu\n");
...
static int gcc_debug_msm8953_probe(struct platform_device *pdev)
{
	...
	struct dentry *setrate, *enable, *measure;

	gcc_base = ioremap(0x1800000, 0x80000);
	if (!gcc_base)
		return -ENOMEM;

	rootdir = debugfs_create_dir("clk-debug", NULL);
	if (IS_ERR_OR_NULL(rootdir))
		return PTR_ERR(rootdir) ?: -ENODATA;

	measure = debugfs_create_dir("measure", rootdir);
	if (IS_ERR_OR_NULL(measure))
		return PTR_ERR(measure) ?: -ENODATA;

	enable = debugfs_create_dir("enable", rootdir);
	if (IS_ERR_OR_NULL(enable))
		return PTR_ERR(enable) ?: -ENODATA;

	setrate = debugfs_create_dir("setrate", rootdir);
	if (IS_ERR_OR_NULL(setrate))
		return PTR_ERR(setrate) ?: -ENODATA;
	...
	if (num_clks) {
		clks = devm_kzalloc(dev, sizeof(*clks) * num_clks, GFP_KERNEL);
