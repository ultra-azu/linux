...
#define FLOOR_TO_CEIL_MAX_RANGE_UV			50000
...
{
	struct cpr_drv *drv = container_of(work,
			struct cpr_drv, restart_work);
	int i;
	...
	for (i = 0; i < 20; i++) {
		u32 cpr_status = cpr_read(drv, CPR3_REG_CPR_STATUS);
		u32 ctl = cpr_read(drv, CPR3_REG_CPR_CTL);
	...
	for (i = 0; i < 20; i++) {
		u32 status = cpr_read(drv, CPR3_REG_IRQ_STATUS);
...
static void cpr_corner_restore(struct cpr_thread *thread, struct corner *corner)
{
	...
	if (drv->desc->version == 4) {
		cpr_masked_write(drv, CPR4_REG_CPR_MASK_THREAD(thread->id),
...
static int cpr_aggregate_voltage(struct cpr_drv *drv)
{
	...
	ret = cpr_pre_voltage(drv, fuse_level, new_uV);
	if (ret)
		return ret;
	...
	ret = cpr_post_voltage(drv, fuse_level, new_uV);
	if (ret)
		return ret;
...
static irqreturn_t cpr_irq_handler(int irq, void *dev)
{
	...
	if (!cpr_ctl_is_enabled(drv)) {
		...
		if (dir != NO_CHANGE) {
			 ...
			 */
			dev_warn(drv->dev, "IRQ occurred for unknown flag (%#08x), restart CPR\n", val);
...
static int cpr_corner_init(struct cpr_thread *thread)
{
	...
	for (apply_scaling = false, i = 0; corner <= end; corner++, i++) {
		...
		corner->min_uV = max(corner->max_uV - FLOOR_TO_CEIL_MAX_RANGE_UV,
				     corner->min_uV);
...
static int cpr_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_threaded_irq(dev, drv->irq, NULL,
					cpr_irq_handler,
